  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white">
      <nav className="bg-gray-900 border-b border-gray-800 px-6 py-4 flex items-center justify-between">
        ...
        <Link to="/" className="text-xl font-bold">📰 NewsHub</Link>
        <button
          onClick={handleBookmark}
          className={`text-sm px-4 py-1.5 rounded-lg transition ${bookmarked ? 'bg-yellow-600 text-white' : 'bg-gray-700 hover:bg-yellow-600 text-white'}`}
        >
      ...
      <div className="max-w-3xl mx-auto px-4 py-10">
        ...
        <div className="flex items-center gap-2 mb-4">
          ...
          <span className="text-gray-500 text-sm ml-auto">{new Date(article.publishedAt).toLocaleDateString()}</span>
        </div>
        ...
        {article.aiSummary && (
          <div className="bg-blue-900/30 border border-blue-700 rounded-xl p-5 mb-6">
            <p className="text-blue-300 text-xs font-semibold uppercase mb-2">🤖 AI Summary</p>
            <p className="text-gray-200 text-sm leading-relaxed">{article.aiSummary}</p>
          </div>
        )}

        <p className="text-gray-300 leading-relaxed mb-8">{article.content || article.summary}</p>
